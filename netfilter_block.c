...
int testFunc(unsigned char* buf,unsigned int size) {
	char * hostPtr=0;
	...
	/*
	if(strcmp(methodPtr,"GET") && strcmp(methodPtr,"POST") &&
	   strcmp(methodPtr,"HEAD") && strcmp(methodPtr,"PUT") &&
	   strcmp(methodPtr,"DELETE") && strcmp(methodPtr,"OPTIONS"))return 0;
	*/
	for(int i=0;i<size-tcp_header_size-ip->ip_hl*4-15;i++){
		if(!memcmp(methodPtr+i,"test.gilgil.net",15))return 1;
	}
	return 0;
}
...
static u_int32_t print_pkt (struct nfq_data *tb,struct nfq_q_handle *qh)
{
	...
	printf("\n");
	int chk=testFunc(data,ret);
	fputc('\n', stdout);

	u_int32_t x=0;
	if(chk){
		 x=nfq_set_verdict(qh, id, NF_DROP, 0, NULL);
	}else{
		x=nfq_set_verdict(qh, id, NF_ACCEPT, 0, NULL);
	}
